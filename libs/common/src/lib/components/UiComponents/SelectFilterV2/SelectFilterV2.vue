  ...
  <div class="select-filter" :class="className">
    ...
    >
      <slot name="data-source" :options="options">
        <a-select-option v-for="(option, index) in options" :key="index" :value="option[source]">
...
import { FILTERS_EQUAL, FILTERS_IN, FILTERS_CONTAINS } from "@/constants";
...
export default {
  ...
  data() {
    ...
    if (this.hasOptionAll && (!this.value.value || !this.value.value.length)) {
      data.inputValue =
        this.mode == "multiple"
  ...
  methods: {
    ...
    onChange(value) {
      ...
      if (this.mode == "multiple") {
        ...
        const obj = this.options.find((option) => option[this.source] == value);

        filter = {
          operator: this.operator,
          value: this.useSourceLabel ? obj[this.sourceLabel] : obj[this.source],
        };
      }
