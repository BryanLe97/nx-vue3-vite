  ...
  methods: {
    setColumnActive() {
      ...
      for (var i = 0; i < this.checkedList.length; i++) {
        const found = this.children.find((e) => e.key === this.checkedList[i]);
        ...
        .then((response) => {
          if (this.listDetailOf) {
            ...
            const indexItem = this.resourceList.findIndex(
              (item) => item[this.resourceIdName] == record[this.resourceIdName]
            );
...
.ant-table-bordered.ant-table-fixed-header .ant-table-scroll .ant-table-header {
  &::-webkit-scrollbar {
