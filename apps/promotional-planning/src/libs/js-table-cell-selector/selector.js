...
export default class Selector {
  ...
  getSelectedRectangleCoords(sc0, sc1) {
    ...
    this.goCells((cell, coord) => {
      ...
      if (scIsAr) {
        if (
          (sc0[0] !== iy || sc0[1] !== this.matrix[iy][ix][2]) &&
          (sc1[0] !== iy || sc1[1] !== this.matrix[iy][ix][2])
        ) {
          return;
        }
      } else {
  ...
  getRectangleCoords(c1, c2) {
    ...
    while (loop) {
      ...
      for (let iy = c2[0]; iy <= c2[0]; iy++) {
        ...
        for (let ix = c1[1]; ix <= c2[1]; ix++) {
          if (
            !isUndef(this.matrix[iy][ix][0]) &&
            this.matrix[iy + 1][ix][0] < 0
          ) {
            c2[0]++;
            loop = true;
            break;
  ...
  initSizeMatrix() {
    ...
    this.matrix = Array(this.countRows)
      .fill()
      .map(() =>
        Array(this.countCols)
          .fill()
          .map(() => Array(2))
      );
  ...
  isIgnoredCell(cell) {
    ...
    return (
      hasClass(cell, this.obApp.obOptions.ignoreClass) || // td
      hasClass(cell.parentNode, this.obApp.obOptions.ignoreClass) || // tr
      (this.obApp.obOptions.ignoreThead && ppn.tagName === "THEAD") ||
      (this.obApp.obOptions.ignoreTfoot && ppn.tagName === "TFOOT")
    );
  }
